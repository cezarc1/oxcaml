...
and print_out_constr ppf constr =
  ...
  in
  let print_attrs ppf =
    List.iter (fun a -> fprintf ppf "@ [@%s]" a.oattr_name) attributes
  in
  match return_type with
  | None ->
      begin match tyl with
      | [] ->
          fprintf ppf "%s%t" name print_attrs
      | _ ->
          fprintf ppf "@[<2>%s of@ %a%t@]" name
            print_out_constr_args tyl print_attrs
      end
  | Some (vars_jkinds, ret_type) ->
      ...
      begin match tyl with
      | [] ->
          fprintf ppf "%a%t@]" print_simple_out_type ret_type print_attrs
      | _ ->
          fprintf ppf "%a -> %a%t@]"
