...
let sort_dedup_modalities_with_locs ~warn l =
...
let transl_modalities_with_default ?(allow_redundant_staticity = false)
  ...
  let modalities_with_loc = sort_dedup_modalities_with_locs ~warn:true annots in
  let open Modality in
  let redundant_modality_allowed (Atom (ax, a)) =
    ...
    | _ -> false
  in
  let warn_redundant loc atom =
    if not (redundant_modality_allowed atom)
...
let sort_dedup_modalities modalities =
  ...
  |> sort_dedup_modalities_with_locs ~warn:false
...
let transl_mod_bounds annots =
  ...
  let nonmodal, base_modality, modal_atoms =
    ...
    |> fun (nm, base, atoms, _) ->
    (* axes listed in the order of implication. *)
    nm, base, sort_dedup_modalities_with_locs ~warn:false (List.rev atoms)
  in
