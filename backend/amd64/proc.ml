...
let hard_int64_reg =
  ...
  |> Array.map (fun phys_gpr_reg ->
     Reg.create_at_location (Naked_int Int64) (Reg phys_gpr_reg))
...
let hard_int16_reg =
  Array.map (Reg.create_alias ~typ:(Naked_int Int16)) hard_int64_reg
let hard_int8_reg =
  Array.map (Reg.create_alias ~typ:(Naked_int Int8)) hard_int64_reg
let hard_vec128_reg =
...
let phys_reg ty (phys_reg : Regs.Phys_reg.t) =
  ...
  | Naked_int Int64 | Addr | Val ->
    ...
    let r = hard_int64_reg.(index_in_class) in
...
let rdi = phys_reg (Naked_int Int64) (P RDI)
let rdx = phys_reg (Naked_int Int64) (P RDX)
let rcx = phys_reg (Naked_int Int64) (P RCX)
let r10 = phys_reg (Naked_int Int64) (P R10)
let r11 = phys_reg (Naked_int Int64) (P R11)
let rbp = phys_reg (Naked_int Int64) (P RBP)
...
let destroyed_at_c_call_win64 =
  ...
  [ Array.map (fun p -> phys_reg (Naked_int Int64) (P p))
...
let operation_supported = function
  ...
  | Creinterpret_cast (Int_of_value | Value_of_int |
                       ...
                       V128_of_vec Vec128)
  | Cstatic_cast (Int_of_int _ | Tagged_int_of_int64 | Int64_of_tagged_int _ |
