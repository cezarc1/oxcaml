...
let register_name typ phys_reg : X86_ast.arg =
  ...
  | Tagged_int | Naked_int _ | Val | Addr -> Reg64 int_reg_name.(reg_index)
...
let phys_rax = phys_reg (Naked_int Int64) (P RAX)

let phys_rdi = phys_reg (Naked_int Int64) (P RDI)

let phys_rdx = phys_reg (Naked_int Int64) (P RDX)

let phys_rcx = phys_reg (Naked_int Int64) (P RCX)
...
let x86_data_type_for_stack_slot : Cmm.machtype_component -> X86_ast.data_type =
  ...
  | Vec512 ->
    ...
    VEC512
  | Valx2 -> VEC128
  | Tagged_int | Naked_int _ | Addr | Val -> QWORD
  | Float32 -> REAL4
  ...
  Reg.Set.iter
    (fun r ->
      ...
      then ()
        ...
        | Val | Addr | Tagged_int | Naked_int _ -> ())
    live;
  if !v512
  then (
    I.require_vec512 ();
...
let record_frame_label live dbg =
  ...
  Reg.Set.iter
    (fun (r : Reg.t) ->
      ...
      | { typ =
            ( Tagged_int | Naked_int _ | Float | Float32 | Vec128 | Vec256
            | Vec512 );
          _
        } ->
        ())
...
let move (src : Reg.t) (dst : Reg.t) =
  ...
  | ( (Tagged_int | Naked_int _ | Val | Addr),
      (Reg _ | Stack _),
      (Tagged_int | Naked_int _ | Val | Addr),
      (Reg _ | Stack _) ) ->
  ...
  | _, (Reg _ | Stack _), _, Unknown ->
    Misc.fatal_errorf
      ...
      src Printreg.reg dst
  | ( ( Float | Float32 | Vec128 | Vec256 | Vec512 | Tagged_int | Naked_int _
      | Val | Addr | Valx2 ),
      (Reg _ | Stack _),
      _,
      _ ) ->
...
let size_of_regs regs =
  Array.fold_right
    (fun r acc ->
      ...
      | Tagged_int | Naked_int _ | Addr | Val -> acc + size_int
      | Float | Float32 ->
    ...
    Array.fold_right
      (fun r (n, offsets) ->
        let next =
          ...
          +
          match r.Reg.typ with
          | Tagged_int | Naked_int _ | Val | Addr -> size_int
          | Float | Float32 ->
            (* Float32 slots still take up a full word *)
            size_float
    ...
    Array.fold_right
      (fun (r : Reg.t) acc ->
        ...
        | Stack (Outgoing k) -> (
          match r.typ with
          | Val -> k :: acc
          | Tagged_int | Naked_int _ | Float | Vec128 | Vec256 | Vec512
          | Float32 ->
            acc
