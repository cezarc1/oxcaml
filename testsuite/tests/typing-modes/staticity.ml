...
(* Functors are often defined without an explicit functor type. The functor
   can still be static, but its return mode then defaults to dynamic, so the
...
(* The same holds when the functor itself is left unannotated. *)
